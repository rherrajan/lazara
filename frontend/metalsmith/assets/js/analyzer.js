	...
	xhttp.onreadystatechange = function() {
		if (this.readyState == 4) {
			if(this.status != 200){
				...
				var analyzeResult = JSON.parse(this.responseText);
				writeAnalyzeResult(analyzeResult, resultContainerTag);
...
function writeAnalyzeResult(analyzeResult, resultContainerTag){

	var row2Tag = resultContainerTag.parentNode.parentNode;
	
	if(analyzeResult.score < 20){
		...
		findigs.innerHTML = "<strong>could not identify the plant</strong>";
	...
	var imgTag = document.createElement('img');
	imgTag.setAttribute('src', analyzeResult.referenceImage);
	imgTag.setAttribute('style', 'width:100%');
	resultContainerTag.appendChild(imgTag);

	var name;
	if(analyzeResult.commonName){
		name = analyzeResult.commonName;
	} else {
		name = analyzeResult.scientificName;
	}
	var judgingClass;
	if(analyzeResult.score > 30){
		judgingClass = 'alert-success';
	} else {
		judgingClass = 'alert-warning';
	}
	var genusHref = "https://en.wikipedia.org/w/index.php?search=" + analyzeResult.genus;
	var speciesHref = "https://pfaf.org/user/Plant.aspx?LatinName=" + analyzeResult.scientificName;

	var findigs = document.createElement('div');
	findigs.setAttribute('class', 'alert ' + judgingClass);
	var species = analyzeResult.family+' => <a href="'+genusHref+'">'+analyzeResult.genus+'</a> => <a href="'+speciesHref+'">'+analyzeResult.scientificName+'</a>';
	findigs.innerHTML = '<strong>'+name+'</strong> <i>('+species+')</i> <br/> Propability: <strong>'+analyzeResult.score+'%</strong>';
	row2Tag.parentNode.insertBefore(findigs, row2Tag.nextSibling);
